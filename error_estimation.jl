...
# https://www.youtube.com/watch?v=E1a6HokwEzw - velocity autocorrelation function ? 
...
using DelimitedFiles
using Printf
using FFTW  # Added for Fast Fourier Transform
...
function autocorrelation(data::Vector{Float64}, max_lag::Int)
    ...
    # 1. Subtract the mean to compute autocovariance
    x = data .- mean(data)
    
    # 2. Zero-pad to the next power of 2 to avoid circular convolution
    pad_length = nextpow(2, 2N - 1)
    padded_x = zeros(Float64, pad_length)
    padded_x[1:N] .= x
    
    # 3. FFT -> Power Spectrum -> Inverse FFT (Wiener-Khinchin theorem)
    F = fft(padded_x)
    S = abs2.(F) # Multiply by complex conjugate
    R = real.(ifft(S))
    
    # 4. Truncate back to length N and apply the correct 1/(N-k) normalization
    acf = R[1:N]
    lags = 0:(N-1)
    acf ./= (N .- lags)
    
    # 5. Normalize by the variance (lag 0) so C(0) = 1.0
    acf ./= acf[1]
    
    # Return up to the requested max_lag (inclusive of lag 0)
...
function compute_stats(data)
    ...
    # FIX: Smooth the plateau estimation instead of taking the maximum.
    # We average the last 10% of the computed block errors to avoid spikes.
    if isempty(errors_block)
        est_err_block = 0.0
    else
        tail_length = max(1, floor(Int, length(errors_block) * 0.1))
        est_err_block = mean(errors_block[end-tail_length+1:end])
    end
    
    # --- Autocorrelation ---
    max_lag = floor(Int, N/5)
    
    # FIX: Replace StatsBase.autocor with our custom, physically accurate ACF
    acf = autocorrelation(data, max_lag)
    
    # Find where ACF roughly hits zero (<= 0.0) to avoid integrating long-tail noise
    cutoff_idx = findfirst(x -> x <= 0.0, acf)
...
# ... [The rest of your plotting and formatting functions remain exactly the same] ...
function generate_subplots(sizes, errors_block, acf, cutoff_idx, naive_error, var_name, ensemble)
