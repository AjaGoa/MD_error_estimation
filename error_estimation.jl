...
Base.@kwdef struct SimParams
    ...
    T_mean_nvt::Float64 = 120.0        # target temperature
    T_mean_npt::Float64 = 120.0 
end

# Goes through all int size of blocks up to length of box N/3
# Calculates mean for each block, error, error of error (FP paper based)
...
# Fits the data to the error_asymptote_model, giving weight/importance based on the amount of block which are being averaged (the larger amount, the more safe)
...
# FP halves the length of the data each step by calculating average of two subsequent values 
# quite not useful for this data size
...
# Identifying pleateau of the block error based on the "range" of error - error of error --> gives boundies where the result is "safe"
# When an error value fits within all of these boundaries of previous block errors, then it can be considerate as a plateau 
...
function identify_plateau(block_sizes, error_block, error_of_errors)
    ...
    max_idx = argmax(error_block) # TODO check waht does this do exactly ?? 
    return error_block[max_idx], block_sizes[max_idx], false
end

# Tuckerman, UMS - autocorrelation based on convolution in frequency and some TODO theorem 
...
# Calculation of acf using explicit double loop for comparison to FFP approach
...
# Calculation of error through statistical inefficiency 
# Problem with acf integration giving negative autoccorelation time - nonsensical for < 1.0, need to find some other approach 
# identified by negative error
function calc_error_acf(tau, N, σ)
    s = 1.0 + 2.0 * tau
    if s < 0.0 
        return  - σ * sqrt(abs(s) / N)  # 
    end 
    return σ * sqrt(s / N)
end
...
# Tuckerman, from partition function equations, units in LAMMPS = real
# error calculated using the error of error from FP approach (or error of variance)
function calculate_cv(E_tot::Vector{Float64}, T_mean::Float64, tau::Float64)
    N = length(E_tot)
    var_E = var(E_tot)
    R = 0.0019872041 # kcal/(mol K)
    Cv = var_E / (R * T_mean^2) # kcal / (K mol)

    # Error in variance: σ_V ≈ V * sqrt(2 * (1 + 2τ) / N)
    s = abs(1.0 + 2.0* abs(tau))
    if tau < 0
        println("τ negative")
        var_E_error = - var_E * sqrt(2 * s / N)
    else
        if tau < 1.0 
            println("τ smaller than 1.0")
        end 
        var_E_error = var_E * sqrt(2 * s / N)
    end 

     # 2 * because its Chi-squared rule for getting the error of variance, goes fro FP error-of-error
...
function calculate_cp(E_pot::Vector{Float64}, E_kin::Vector{Float64}, Press::Vector{Float64}, Dens::Vector{Float64}, T_mean::Float64, tau::Float64, params::SimParams)
    ...
    #cm^3 to m^3
    Vol_m3 = (mass_g ./ Dens) .* 1e-6
    
    # Enthalpy H = E + PV 
    # 1 atm = 101325 N / m^2 * m^3 = J , 4.184 J = 1 cal 
    # 101325 (to Pa) / 4184 (to kcal) / mol ( n = N atoms / N_A) -- > to Pa * N_A / (to kcal * N_atoms)
    unit_conversion = 101325 * N_A / (4184*params.N_atoms)
    PV_kcal = Press .* Vol_m3 .* unit_conversion
    ...
    # Error in variance
    s = abs(1.0 + 2.0 * tau) # tohle si nejsem jista jestli je legalni ? 
...
function plot_heat_capacity(Cp, Cv, Cp_error, Cv_error)
    labels = ["Cp", "Cv"]
    vals = [Cp, Cv]
    errs = [Cp_error, Cv_error]
    
    # subplot for heat capacities values
    p1 = bar(labels, vals, 
    ...
    for (i, v) in enumerate(vals)
        annotate!(p1, i, v / 2 , text(string(round(v, digits=4)), 12, :center, rotation=90))
    end
    ...
    # Annotate the errors above the bars
    for (i, e) in enumerate(errs)
        annotate!(p2, i, e/2, text(string(round(e, digits=4)), 12, :center, rotation=90))
    end
    
    final_plot = plot(p1, p2, layout=(1, 2), size=(800, 400), margin=5Plots.mm)
    
    savefig(final_plot, "heat_capacity.svg")
end
...
function compute_stats(data, block::NamedTuple, acf::NamedTuple)
    ...
    tau_acf =  sum(acf.acf[2:end]) # TODO need for better treatment
    tau_acf_loop = sum(acf.acf_loop[2:end])

    # Calculate τ equivalents from block methods
    taus = [
        0.5 * ((block.true_error_estimate_blockAvg / naive_error)^2 - 1.0), # TODO ozdrojovat
...
function plotting(N, ensemble_name, name, dt, block::NamedTuple, acf::NamedTuple, stats::NamedTuple)
    p1 = plot(block.block_sizes_blockAvg .* dt, block.block_errors_blockAvg, 
            ...
            legendfontsize = 12.0, titlefontsize = 15.0, guidefontsize = 15.0, tickfontsize = 12.0)
    ...
    bar_labels = text.(round.(method_values, sigdigits=3), 10, :bottom)
    
    p3 = bar(method_labels, method_values, legend=false, ylabel="Estimated std. error", title="Error Estimate Comparison", xrotation=90, 
    series_annotations=bar_labels, top_margin=25Plots.mm, legendfontsize = 12.0, titlefontsize = 15.0, guidefontsize = 15.0, tickfontsize = 12.0)
...
function compare_ensembles(nvt_results::Dict, npt_results::Dict)
    ...
    for prop_name in common_props
        ...
        # Omit tau_acf from purely error comparisons # TODO check !
...
function plot_ensemble_comparison(nvt_results::Dict, npt_results::Dict)
    common_props = intersect(keys(nvt_results), keys(npt_results))
    
    label_map = Dict(
        :naive_error          => "Naive",
        :block_fit            => "Blk(fit)",
        :block_fp_plateau     => "FP(plat)",
        :block_plateau        => "Blk(plat)",
        :block_tau_fit        => "Blk(fit) τ",
        :block_tau_plateau    => "Blk(plat) τ",
        :block_tau_fp_plateau => "FP(plat) τ",
        :acf_fft              => "ACF(FFT)",
        :acf_loop             => "ACF(loop)"
    )
    ...
    for prop_name in common_props
        ...
        keys_to_plot = filter(k -> k != :tau_acf, keys(nvt_errors))
        x_labels = [get(label_map, k, string(k)) for k in keys_to_plot]
        
        nvt_vals = [getproperty(nvt_errors, k) for k in keys_to_plot]
        npt_vals = [getproperty(npt_errors, k) for k in keys_to_plot]
        
        # 1. Define base numeric positions for the x-axis
        x_base = 1:length(keys_to_plot)
        
        # 2. Define width and spacing
        bw = 0.35      # bar width
        offset = 0.18  # distance from center
        
        # 3. Create annotations (rounded to 3 sigdigits, rotated 90 degrees to prevent overlapping)
        # :left alignment ensures the text starts exactly at the top edge of the bar and writes upwards
        nvt_anno = text.(round.(nvt_vals, sigdigits=3), 9, :left, rotation=90)
        npt_anno = text.(round.(npt_vals, sigdigits=3), 9, :left, rotation=90)
        
        # 4. Initialize an empty plot with formatting
        p = plot(
            title="Ensemble Error Comparison: $prop_name",
            ylabel="Estimated std. error",
            xlabel="Error Method",
            legend=:outertopright,
            size=(1000, 600),
            bottom_margin=25Plots.mm,
            top_margin=30Plots.mm,     # Significantly increased so rotated labels fit inside the canvas
        ...
        bar!(p, x_base .- offset, nvt_vals, label="NVT", bar_width=bw, color=:steelblue, alpha=0.8, series_annotations=nvt_anno)
        bar!(p, x_base .+ offset, npt_vals, label="NpT", bar_width=bw, color=:darkorange, alpha=0.8, series_annotations=npt_anno)
        
        xticks!(p, x_base, x_labels, xrotation=45)
        
        filename = "ensemble_comparison_$(prop_name).svg"
        savefig(p, filename)
    end
end

plot_ensemble_comparison(nvt_results, npt_results)

function heat_capacity(nvt_raw, npt_raw, nvt_results, npt_results)
    println("Heat capacity calculation:")
    println("="^102)
    ...
    return Cp, Cv, Cp_err, Cv_err
end

Cp, Cv, Cp_err, Cv_err = heat_capacity(nvt_raw, npt_raw, nvt_results, npt_results)
plot_heat_capacity(Cp, Cv, Cp_err, Cv_err)
